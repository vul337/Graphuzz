...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  if (afl->expand_corpus) {
    //u8 *trace_mini_tmp = ck_alloc((afl->fsrv.map_size >> 3));
    //minimize_bits(afl, trace_mini_tmp, afl->fsrv.trace_bits);
    u64 testcase_hash = hash64(afl->fsrv.trace_bits, afl->fsrv.map_size, HASH_CONST);
    u8* f_seed_name = alloc_printf("%s/%016llx.seed", afl->trainingSetExpandDir, testcase_hash);
    if (access(f_seed_name, F_OK)) {
      int fd_seed = open(f_seed_name, O_RDWR | O_CREAT, DEFAULT_PERMISSION);
      if (fd_seed < 0) {
        FATAL("seed file create error %s", f_seed_name);
      }
      ck_write(fd_seed, mem, len, f_seed_name);
      close(fd_seed);
    }
    //ck_free(trace_mini_tmp);
    ck_free(f_seed_name);
    //trace_mini_tmp = NULL;
    f_seed_name = NULL;
  }
