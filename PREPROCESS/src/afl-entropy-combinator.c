...
void copy_testcase(afl_state_t *afl, char *fn, size_t N, size_t count) {
  char * dir = alloc_printf("%s/%lu_%lu", afl->trainingSetOutputDir, N, count);
  if (access(dir, F_OK)) {
    mkdir(dir, 0766);
  }
  char *corpus_path = alloc_printf("%s/corpus", dir);
  FILE *f_corpus = NULL;
  if (access(corpus_path, F_OK)) {
    f_corpus = fopen(corpus_path, "w+");
  }
  else {
    f_corpus = fopen(corpus_path, "a+");
  }
  ck_free(corpus_path);
  ck_free(dir);
  fprintf(f_corpus, "%s\n", strrchr(fn, '/')+1);
...
double get_entropy_score(afl_state_t *afl, u8* now_bits) {
  double AVG = 0;
  double variance = 0;
  for (size_t i = 0; i < afl->fsrv.real_map_size; i++) {
    AVG += now_bits[i];
  }
  AVG /= (double)(afl->fsrv.real_map_size);
  for (size_t i = 0; i < afl->fsrv.real_map_size; i++) {
    variance += ((double)now_bits[i] - AVG) * ((double)now_bits[i] - AVG);
  }
  return sqrt(variance);
}

void entropy_combinator(afl_state_t *afl, size_t count, size_t N) {
  ...
  u8 *entropy_tmp = calloc(1, afl->fsrv.real_map_size);
  u8 *entropy_ori = calloc(1, afl->fsrv.real_map_size);
  u8 *entropy_now = calloc(1, afl->fsrv.real_map_size);
  u8 *entropy_first = calloc(1, afl->queued_items);
  u8 *entropy_rnd = calloc(1, afl->queued_items);
  size_t *entropy_max_idxs = calloc(N, sizeof(size_t));
  size_t *selected = calloc(N, sizeof(size_t));
  if (entropy_tmp == NULL) {
    FATAL("tmp bitmap failed!\n");
  }

  while (output_cnt <= count) {

    while (1) {
      int n = read(rdfd, &rand_idx, 8);
      if (n != 8) {
        FATAL("read error!\n");
      }
      rand_idx %= afl->queued_items;
      if (entropy_first[rand_idx]) {
        continue;
      }
      q = afl->queue_buf[rand_idx];
      s32 _fd = open(q->fname, O_RDONLY);
      if (_fd < 0) { PFATAL("Unable to open '%s'", q->fname); }
      u32 fst_read_len = MIN(q->len, (u32)MAX_FILE);
      use_mem = afl_realloc(AFL_BUF_PARAM(in), fst_read_len);
      ck_read(_fd, use_mem, fst_read_len, q->fname);
      close(_fd);
      s32 _res = calibrate_case(afl, q, use_mem, 0, 1);
      if (_res == FSRV_RUN_ERROR) {
        FATAL("run error! %d\n", _res);
      }
      memcpy(entropy_ori, afl->fsrv.trace_bits, afl->fsrv.real_map_size);
      selected[0] = rand_idx;
      break;
    }
    ...
    memset(entropy_rnd, 0, afl->queued_items);
    while (round_max < ((20 / N) > 0 ? (20 / N) : 1)&& round_max < afl->queued_items) {
retry:
      memcpy(entropy_tmp, entropy_ori, afl->fsrv.real_map_size);
      memset(selected + 1, 0, sizeof(size_t) * (N-1));
      size_t i = 1;
      while (i < N) {
        read(rdfd, &idx, 8);
        idx %= afl->queued_items;
        if (entropy_rnd[idx] == 0) {
          entropy_rnd[idx] = 0;
        }
        else {
          continue;
      ...
      for (size_t iii = 0; iii < N; iii ++) {
        ...
        for (size_t j = 0 ; j < afl->fsrv.real_map_size; j++) {
          entropy_tmp[j] = ((double)entropy_tmp[j] / (double)(iii+2) * (double)(iii+1)) + (double)afl->fsrv.trace_bits[j] / (double)(iii+2);
        }
      }
      double entropy = get_entropy_score(afl, entropy_tmp);
      //WARNF("now corpus score: %lf\n", entropy);
      //for (size_t z = 0; z < N; z++) {
      //  printf("%lu\t", selected[z]);
      //}
      printf("\n");
      if (entropy > max_entropy) {
        max_entropy = entropy;
        memcpy(entropy_max_idxs, selected, sizeof(size_t) * N);
        added = 1;
      }
      round_max ++;
    }
    if (added == 0) {
      WARNF("no more corpus found!\n");
      goto retry;
    }
    else {
      for (size_t i = 0; i < N; i++) {
        copy_testcase(afl, afl->queue_buf[entropy_max_idxs[i]]->fname, N, output_cnt);
      }
      WARNF("corpus score: %lf\n", max_entropy);
      entropy_first[entropy_max_idxs[0]] = 1;
      output_cnt ++;
    }
  }

  close(rdfd);
  free(entropy_max_idxs);
  free(selected);
  free(entropy_now);
  free(entropy_first);
  free(entropy_tmp);
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
      (opt = getopt(
           argc, argv,
           "+Ab:B:c:CdDe:E:hi:I:f:F:g:G:l:L:m:M:nNOo:p:RQs:S:t:T:UV:WXx:YZ:K:u:v:")) >
      0) {

    switch (opt) {

      case 'u':
        round_min = (size_t)atoll(optarg);
        break;
      
      case 'v':
        round_max = (size_t)atoll(optarg);
        break;

      case 'K':
        afl->trainingSetOutputDir = optarg;
        break;
  ...
  if (likely(!afl->afl_env.afl_no_startup_calibration)) {
    ...
    for (size_t N=round_min; N<round_max; N++) {
      entropy_combinator(afl, 100, N);
    }
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
//      if ((get_cur_time() - afl->start_time) > (10 * 60 * 1000)) {
//        exportTrainingSet();
//      }
