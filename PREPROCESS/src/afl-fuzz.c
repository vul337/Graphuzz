...
struct queue_entry *target_queue = NULL;
u8* first_virgin = NULL;

void exportTrainingSet(afl_state_t *afl) {
  char *first_virgin_path = alloc_printf("%s/first_virgin", afl->trainingSetOutputDir);
  char *last_virgin_path = alloc_printf("%s/last_virgin", afl->trainingSetOutputDir);
  int fd = open(first_virgin_path, O_RDWR | O_CREAT, DEFAULT_PERMISSION);
  if (fd < 0) {
    FATAL("exportTrainingSet fail [%s]\n", first_virgin_path);
  }
  ck_write(fd, first_virgin, afl->fsrv.map_size, first_virgin_path);
  close(fd);
  fd = open(last_virgin_path, O_RDWR | O_CREAT, DEFAULT_PERMISSION);
  if (fd < 0) {
    FATAL("exportTrainingSet fail [%s]\n", last_virgin_path);
  }
  ck_write(fd, afl->virgin_bits, afl->fsrv.map_size, last_virgin_path);
  close(fd);
  ck_free(first_virgin_path);
  ck_free(last_virgin_path);
}
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
    ...
    switch (opt) {
      ...
      case 'J':
        afl->trainingSetExpandDir = optarg;
        afl->expand_corpus = 1;
        break;
  ...
  first_virgin = calloc(1, afl->fsrv.real_map_size);
  memcpy(first_virgin, afl->virgin_bits, afl->fsrv.real_map_size);
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      if ((get_cur_time() - afl->start_time) > (30 * 60 * 1000)) {
        exportTrainingSet(afl);
        goto stop_fuzzing;
      }
