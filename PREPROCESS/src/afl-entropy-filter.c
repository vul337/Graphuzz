...
void copy_testcase(afl_state_t *afl, char *fn) {
 char * cmd = alloc_printf("cp %s %s/%s", fn, afl->trainingSetOutputDir, strrchr(fn, '/'));
 system(cmd);
 free(cmd);
}

double get_entropy_score(afl_state_t *afl, u8* now_bits, u8* new_bits) {
  double score = 0;
  for (size_t i = 0; i < afl->fsrv.real_map_size; i++) {
    score += (double)(now_bits[i] - new_bits[i]) * (double)(now_bits[i] - new_bits[i]) / (double)(now_bits[i]+1);
  }
  return sqrt((double)score);
}

void entropy_filter(afl_state_t *afl, size_t count) {
  ...
  u8 *entropy_idx = calloc(1, afl->queued_items);
  ...
  if (_res == FSRV_RUN_ERROR || _res == FSRV_RUN_NOBITS) {
    FATAL("run error!\n");
  }
  memcpy(entropy_tmp, afl->fsrv.trace_bits, afl->fsrv.real_map_size);
  ...
  while (output_cnt < count) {
    ...
    while (round_max < 100 && round_max < afl->queued_items) {
      read(rdfd, &idx, 8);
      ...
      double distance = get_entropy_score(afl, entropy_tmp, afl->fsrv.trace_bits);
      WARNF("seed %s score: %lf\n", q->fname, distance);
      if (distance > max_distance && entropy_idx[idx] == 0) {
        memcpy(entropy_now, afl->fsrv.trace_bits, afl->fsrv.real_map_size);
        now_idx = idx;
    ...
    if (added == 0) {
      FATAL("wanted output filter count cannot be satisfied!\n");
    }
    else {
      for (size_t i = 0 ; i < afl->fsrv.real_map_size; i++) {
        entropy_tmp[i] = (u8)((double)entropy_tmp[i] / (output_cnt+1) * (output_cnt)) + entropy_now[i] / (output_cnt+1);
      }
      copy_testcase(afl, afl->queue_buf[now_idx]->fname);
      entropy_idx[now_idx] = 1;
      output_cnt ++;
    }
  }
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
      (opt = getopt(
           argc, argv,
           "+Ab:B:c:CdDe:E:hi:I:f:F:g:G:l:L:m:M:nNOo:p:RQs:S:t:T:UV:WXx:YZ:K:J")) >
      0) {
