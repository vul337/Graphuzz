...
uint32_t setBBName(BasicBlock &BB) {
  uint32_t bb_name = AFL_R(MAP_SIZE);
  BB.setName("bb__" + std::to_string(bb_name) + "__");
  if (!BB.hasName() || strncmp(BB.getName().str().c_str(), "bb__", 4)) {
    std::string newname = "bb__" + std::to_string(bb_name);
    Twine t(newname);
  ...
  if (bb_name != atoll(BB.getName().str().c_str()+4)) {
    WARNF("BB NOT SAME! :%d %ld", bb_name, atoll(BB.getName().str().c_str()+4));
    std::cerr << BB.getName().str() << std::endl;
  }
  return bb_name;
}

void logBBFeature(Module &M, llvm::Function &F, llvm::BasicBlock  &BB) {
  std::ofstream ofs;
  char *cdf_dir_path = getenv("CDF_FILE_DIR");
  if (!cdf_dir_path) {
    return;
  }
  std::string src_file_path = M.getSourceFileName();
  std::string fpath;
  if (src_file_path.find('/') != src_file_path.npos) {
    fpath = std::string(cdf_dir_path) + src_file_path.substr(src_file_path.rfind('/')) + ".bbf";
  }
  else {
    fpath = std::string(cdf_dir_path) + "/" + src_file_path + ".bbf";
  }
  ofs.open(fpath.c_str(), std::ios::app);
  //feature
  size_t alloc_cnt = 0;
  size_t ir_cnt = 0;
  size_t binaryop_cnt = 0;
  for(auto II = BB.begin(), IE = BB.end(); II != IE; ++II)
  {
    ir_cnt ++;
    //II->print(llvm::outs(), false);
    //std::cerr << std::endl;
    if (isa<AllocaInst>(II)) {
      ...
      if (ci->getCalledFunction() != NULL) {
        if (ci->getCalledFunction()->hasName()) {
          if ( ci->getCalledFunction()->getName().str().find("alloc") != std::string::npos ||
            isBlacklisted(ci->getCalledFunction())) {
            alloc_cnt ++;
          }
        }
      }
    }
    else if (II->isBinaryOp()){
      binaryop_cnt ++;
    }
  }
  ofs << M.getSourceFileName() << ":" << F.getName().str() << ":" << (unsigned int)atoll(BB.getName().str().c_str()+strlen("bb__"));
  ofs << ":" << alloc_cnt << ":" << binaryop_cnt << ":" << ir_cnt;
  ofs << std::endl;
  ofs.close();
}

void logCFG(Module &M, llvm::Function &F, uint32_t src, uint32_t dst) {
  std::ofstream ofs;
  char *cdf_dir_path = getenv("CDF_FILE_DIR");
  if (!cdf_dir_path) {
    return;
  ...
  std::string fpath;
  if (src_file_path.find('/') != src_file_path.npos) {
    fpath = std::string(cdf_dir_path) + src_file_path.substr(src_file_path.rfind('/')) + ".cfg";
  }
  else {
    fpath = std::string(cdf_dir_path) + "/" + src_file_path + ".cfg";
  }
  ofs.open(fpath.c_str(), std::ios::app);
  ofs << M.getSourceFileName() << ":" << F.getName().str() << ":" << src << ":" << dst << std::endl;
  ofs.close();
}

void logCG(Module &M, llvm::Function &caller, llvm::Function &callee, uint32_t src, std::string &lvar, std::vector<std::string> &call_args) {
  std::ofstream ofs;
  char *cdf_dir_path = getenv("CDF_FILE_DIR");
  if (!cdf_dir_path) {
    return;
  ...
  std::string fpath;
  if (src_file_path.find('/') != src_file_path.npos) {
    fpath = std::string(cdf_dir_path) + src_file_path.substr(src_file_path.rfind('/')) + ".cg";
  }
  else {
    fpath = std::string(cdf_dir_path) + "/" + src_file_path + ".cg";
  }
  ofs.open(fpath.c_str(), std::ios::app);
  ofs << M.getSourceFileName() << ":" << caller.getName().str() << ":" << callee.getName().str() << ":" << src << ":" << lvar;
...
void logRG(Module &M, llvm::Function &callee, uint32_t src, std::string &valuename) {
  std::ofstream ofs;
  char *cdf_dir_path = getenv("CDF_FILE_DIR");
  if (!cdf_dir_path) {
    return;
  ...
  std::string fpath;
  if (src_file_path.find('/') != src_file_path.npos) {
    fpath = std::string(cdf_dir_path) + src_file_path.substr(src_file_path.rfind('/')) + ".rg";
  }
  else {
    fpath = std::string(cdf_dir_path) + "/" + src_file_path + ".rg";
  }
  ofs.open(fpath.c_str(), std::ios::app);
  ofs << M.getSourceFileName() << ":" << callee.getName().str() << ":" << src << ":" << valuename;
  ofs << std::endl;
  ofs.close();
}

void logDG(Module &M, char *json_content) {
  std::ofstream ofs;
  char *cdf_dir_path = getenv("CDF_FILE_DIR");
  if (!cdf_dir_path) {
    return;
  ...
  std::string fpath;
  if (src_file_path.find('/') != src_file_path.npos) {
    fpath = std::string(cdf_dir_path) + src_file_path.substr(src_file_path.rfind('/')) + ".dg";
  }
  else {
    fpath = std::string(cdf_dir_path) + "/" + src_file_path + ".dg";
  }
  ofs.open(fpath.c_str(), std::ios::app);
  ofs << json_content << std::endl;
  ofs.close();
}

void logDCG(Module &M, char *json_content) {
  std::ofstream ofs;
  char *cdf_dir_path = getenv("CDF_FILE_DIR");
  if (!cdf_dir_path) {
    return;
  ...
  std::string fpath;
  if (src_file_path.find('/') != src_file_path.npos) {
    fpath = std::string(cdf_dir_path) + src_file_path.substr(src_file_path.rfind('/')) + ".dcg";
  }
  else {
    fpath = std::string(cdf_dir_path) + "/" + src_file_path + ".dcg";
  }
...
{
  std::string ret_val = "noname";
  if(val->getName().empty()) {
    if (isa<ConstantInt>(val)) {
      ret_val = "#" + llvm::dyn_cast<llvm::ConstantInt>(val)->getValue().toString(10, true);
    }
    else if (isa<ConstantFP>(val)) {
      ret_val = "#" + std::to_string(::dyn_cast<llvm::ConstantFP>(val)->getValueAPF().convertToDouble());
    }
    else {
      ret_val = "#?";
    }
...
int getSlotNum(Module &M, Instruction *II, llvm::Value *V, ModuleSlotTracker &MST) {
  V->print(llvm::nulls(), MST, false);
  return MST.getLocalSlot(V);
}

std::string getValueName(Module &M, Instruction *II, llvm::Value *V, ModuleSlotTracker &MST) {
  //II->print(llvm::outs(), false);
  //llvm::outs()<<"\n";
  int slotnum = getSlotNum(M, II, V, MST);
  //std::cerr<< "slotnum:" << slotnum << std::endl;
  if (slotnum != -1) {
    return std::string("%") + std::to_string(slotnum);
  }
  else {
    return getvaluestaticname(V);
  }
}
...
public:
  ...
  long type;
public:
  var_desc(std::string var, long type) {
    this->var = var;
    this->type = type;
  }
};
...
std::vector<dataflow_vec> dataflow;

class cfg_vec {
  public:
  std::string BBsrc;
  std::string BBvar;
  std::vector<std::string> BBdst;
};

std::vector<cfg_vec> cfgflow;
  ...
  */
  ...
  for (auto &F : M) {
    ...
    for (auto &BB : F) {
      unsigned int cur_loc = 0;
      if (!BB.hasName() || strncmp(BB.getName().str().c_str(), "bb__", 4)) {
        cur_loc = setBBName(BB);
      }
      else {
        cur_loc = (unsigned int)atoll(BB.getName().str().c_str()+strlen("bb__"));
      }
      logBBFeature(M, F, BB);
      for (succ_iterator BB_succ = llvm::succ_begin(&BB), E = llvm::succ_end(&BB); BB_succ != E; ++BB_succ) {
        uint32_t dst_loc = 0;
        if (!BB_succ->hasName() || strncmp(BB_succ->getName().str().c_str(), "bb__", 4)) {
          dst_loc = setBBName(**BB_succ);
        }
        else {
          dst_loc = (unsigned int)atoll(BB.getName().str().c_str()+strlen("bb__"));
        }
        logCFG(M, F, cur_loc, dst_loc);
        if (cur_loc > MAP_SIZE || dst_loc > MAP_SIZE) {
          WARNF("%s -> %s\n", BB.getName().str().c_str(), BB_succ->getName().str().c_str());
        }
      }
      ...
      if (instrument_ctx && &BB == &F.getEntryBlock()) {
        ...
        for (auto &it: BB) {
          if ((unsigned int)atoll(BB.getName().str().c_str()+strlen("bb__")) == 0) {
            break;
          }
          if (isa<CallInst>(&it)) {
            ...
            if (ci.getCalledFunction() != NULL) {
              if (ci.getCalledFunction()->hasName()) {
                if ( ci.getCalledFunction()->getName().str().find("__afl_log") != std::string::npos ||
                  isBlacklisted(ci.getCalledFunction())) {
                  continue;
                }
              }
            }
            //IRBuilder<> IRB(it.getNextNonDebugInstruction());
            //unsigned int cur_loc = (unsigned int)atoll(BB.getName().str().c_str()+strlen("bb__"));
            //ConstantInt *CurLoc = ConstantInt::get(Int32Ty, cur_loc);
            //IRB.CreateCall(AFLLOG, {CurLoc});
          }
        }
...
/* There is a problem with Ubuntu 18.04 and llvm 6.0 (see issue #63).
  ...
  */
      ...
      for(auto II = BB->begin(), IE = BB->end(); II != IE; ++II)
      {
        ...
        if (isa<CallInst>(II)) {
          ...
          for (size_t i = 0; i < ci->getNumArgOperands(); i ++) {
            std::string varname = getValueName(M, llvm::dyn_cast<llvm::Instruction>(II), ci->getArgOperand(i), MST);
            if (varname[0] == '%') {
              varname.insert(0, FI->getName().str());
            }
            call_args.push_back(
              varname
            );
          ...
          if (ci->getCalledFunction() != NULL && ci->getCalledFunction()->hasName()) {
            //std::cerr << "CallInst Found: " << F.getName().str() << " --> " << ci.getCalledFunction()->getName().str() << std::endl;
            unsigned int cur_loc = 0;
            if (!BB->hasName() || strncmp(BB->getName().str().c_str(), "bb__", 4)) {
              cur_loc = setBBName(*BB);
            }
            ...
            if (ci->getCalledFunction()->hasExactDefinition()) {
              std::string lvar = getValueName(M, llvm::dyn_cast<llvm::Instruction>(II), llvm::dyn_cast<llvm::Value>(II), MST);
              if (lvar[0] == '%') {
                lvar =  FI->getName().str()  + lvar;
                logCG(M, *FI, *ci->getCalledFunction(), cur_loc, lvar, call_args);
              }
            }
          }
          continue;
          ...
          ReturnInst *ri = cast<ReturnInst>(II);
          if (ri->getReturnValue() != NULL) {
            std::string varname = getValueName(M, llvm::dyn_cast<llvm::Instruction>(II), ri->getReturnValue(), MST);
            if (varname[0] == '%') {
              varname.insert(0, FI->getName().str());
            }
            logRG(M, *FI, cur_loc, varname);
          }
        }
        else {
          std::string lvar = getValueName(M, llvm::dyn_cast<llvm::Instruction>(II), llvm::dyn_cast<llvm::Value>(II), MST);
          if (lvar[0] == '#') {
            ...
            llvm::Value* storevalptr;
            llvm::Value* storeval;
            //II->print(llvm::outs(), false);
            //std::cerr << std::endl;
            switch(II->getOpcode()) {
              case llvm::Instruction::Store:
                storeinst = llvm::dyn_cast<llvm::StoreInst>(II);
                storevalptr = storeinst->getPointerOperand();
                storeval    = storeinst->getValueOperand();
                lvar = getValueName(M, llvm::dyn_cast<llvm::Instruction>(II), storevalptr, MST);
                if (lvar[0] == '%') {
                  vars.vars.push_back(
                    var_desc(
                      FI->getName().str()  + lvar,
                      (long)(storevalptr->getType()->getTypeID())
                    )
                  );
                }
                else {
                ...
                rvar = getValueName(M, llvm::dyn_cast<llvm::Instruction>(II), storeval, MST);
                if (rvar[0] == '%') {
                  vars.vars.push_back(
                    var_desc(
                      FI->getName().str()  + rvar,
                      (long)(storeval->getType()->getTypeID())
                    )
                  );
                }
                else {
                ...
                vars.BBName = BB->getName().str();
                vars.inst = II->getOpcodeName();
                dataflow.push_back(vars);
                break;
              case llvm::Instruction::Br:
                ...
                {
                  cfg_vec cfg_info;
                  cfg_info.BBsrc = BB->getName().str();
                  brinst = llvm::dyn_cast<llvm::BranchInst>(II);
                  ...
                  for (size_t i = 0; i < brinst->getNumSuccessors(); i++) {
                    if (!brinst->getSuccessor(i)->hasName() || strncmp(brinst->getSuccessor(i)->getName().str().c_str(), "bb__", 4)) {
                      setBBName(*brinst->getSuccessor(i));
                    }
                    cfg_info.BBdst.push_back(brinst->getSuccessor(i)->getName().str());
                    std::string varname = getValueName(M, llvm::dyn_cast<llvm::Instruction>(II), brinst->getOperand(0), MST);
                    if (varname[0] == '%') {
                      varname.insert(0, FI->getName().str());
                    }
                    cfg_info.BBvar = varname;
                  }
                  cfgflow.push_back(cfg_info);
                }
                break;
              case llvm::Instruction::Switch:
                ...
                //std::cerr << std::endl;
                {
                  cfg_vec cfg_info;
                  cfg_info.BBsrc = BB->getName().str();
                  switchinst = llvm::dyn_cast<llvm::SwitchInst>(II);
                  for (size_t i = 0; i < switchinst->getNumSuccessors(); i++) {
                    if (!switchinst->getSuccessor(i)->hasName() || strncmp(switchinst->getSuccessor(i)->getName().str().c_str(), "bb__", 4)) {
                      setBBName(*switchinst->getSuccessor(i));
                    }
                    cfg_info.BBdst.push_back(switchinst->getSuccessor(i)->getName().str());
                    std::string varname = getValueName(M, llvm::dyn_cast<llvm::Instruction>(II), switchinst->getOperand(0), MST);
                    if (varname[0] == '%') {
                      varname.insert(0, FI->getName().str());
                    }
                    cfg_info.BBvar = varname;
            ...
            if (lvar[0] == '%') {
              vars.vars.push_back(
                var_desc(
                  FI->getName().str()  + lvar,
                  (long)(llvm::dyn_cast<llvm::Value>(II)->getType()->getTypeID())
                )
              );
            }
            else {
            ...
            for (size_t i = 0; i < II->getNumOperands(); i++) {
              std::string varname = getValueName(M, llvm::dyn_cast<llvm::Instruction>(II), II->getOperand(i), MST);
              if (varname[0] == '%') {
                varname.insert(0, FI->getName().str());
              }
              vars.vars.push_back(
                var_desc(
                  FI->getName().str()  + lvar,
                  (long)(II->getOperand(i)->getType()->getTypeID())
                )
              );
            }
            if (!BB->hasName() || strncmp(BB->getName().str().c_str(), "bb__", 4)) {
  ...
  rapidjson::StringBuffer buf_dataflow;
  rapidjson::PrettyWriter<rapidjson::StringBuffer> writer_dataflow(buf_dataflow);
  writer_dataflow.StartObject();
  writer_dataflow.Key("dataflow");
  writer_dataflow.StartArray();
  for (size_t i = 0; i < dataflow.size(); i++) {
    writer_dataflow.StartObject();
    writer_dataflow.Key("BB"); writer_dataflow.String(dataflow[i].BBName.c_str());
    writer_dataflow.Key("OPCode"); writer_dataflow.String(dataflow[i].inst.c_str());
    writer_dataflow.Key("lvar"); 
      writer_dataflow.StartObject();
        writer_dataflow.Key("Name");
        writer_dataflow.String(dataflow[i].vars[0].var.c_str());
        writer_dataflow.Key("Type");
        writer_dataflow.String(std::to_string(dataflow[i].vars[0].type).c_str());
      writer_dataflow.EndObject();
    writer_dataflow.Key("rvar");
    writer_dataflow.StartArray();
    for (size_t j = 1; j < dataflow[i].vars.size(); j ++) {
      writer_dataflow.StartObject();
        writer_dataflow.Key("Name");
        writer_dataflow.String(dataflow[i].vars[j].var.c_str());
        writer_dataflow.Key("Type");
        writer_dataflow.String(std::to_string(dataflow[i].vars[j].type).c_str());
      writer_dataflow.EndObject();
    }
    writer_dataflow.EndArray();
    writer_dataflow.EndObject();
  }
  writer_dataflow.EndArray();
  writer_dataflow.EndObject();
  const char* json_content = buf_dataflow.GetString();
  logDG(M, (char *)json_content);
  //std::cerr << std::endl;

  rapidjson::StringBuffer buf_cfgflow;
  rapidjson::PrettyWriter<rapidjson::StringBuffer> writer_cfgflow(buf_cfgflow);
  writer_cfgflow.StartObject();
  writer_cfgflow.Key("dcgflow");
  writer_cfgflow.StartArray();
  for (size_t i = 0; i < cfgflow.size(); i++) {
    writer_cfgflow.StartObject();
    writer_cfgflow.Key("BBsrc"); writer_cfgflow.String(cfgflow[i].BBsrc.c_str());
    writer_cfgflow.Key("BBvar"); writer_cfgflow.String(cfgflow[i].BBvar.c_str());
    writer_cfgflow.Key("BBdst");
    writer_cfgflow.StartArray();
    for (size_t j = 0; j < cfgflow[i].BBdst.size(); j ++) {
      writer_cfgflow.String(cfgflow[i].BBdst[j].c_str());
    }
    writer_cfgflow.EndArray();
    writer_cfgflow.EndObject();
  }
  writer_cfgflow.EndArray();
  writer_cfgflow.EndObject();
  json_content = buf_cfgflow.GetString();
  logDCG(M, (char *)json_content);
  //std::cerr << json_content;
