...
class CDFG:
    ...
    def exportTrainingSet(self, seeds_dir: str, export_dir: str):

        all_seeds = set()
        for seed in os.listdir(seeds_dir):
            all_seeds.add(seed)
        random.seed(time.time())
        for N in [2**x for x in range(1, 14)]:
            print("Handling N: {}".format(N))
            for M in tqdm.tqdm(range(0, 8)):
                first_virgin_combine = None
                seeds_now = random.sample(all_seeds, N)
                for seed in seeds_now:
                    if first_virgin_combine == None:
                        with open(os.path.join(seeds_dir, seed, "first_virgin"), "rb") as fvd:
                ...
                dgl_corpus_base = self.base_graph.clone()
                ndatas = dgl_corpus_base.ndata['attr']
                ...
                edges =  dgl_corpus_base.edges(form = 'uv')
                for i in range(0, len(edges[0])):
                    src = self.id_bb_dict[edges[0][i].item()]
                    dst = self.id_bb_dict[edges[1][i].item()]
                    hash_val = (src >> 1) ^ dst
                    if first_virgin_combine[hash_val] != 0xff:
                        corpus_covered.add(edges[0][i].item())
                        corpus_covered.add(edges[1][i].item())

                nei_nodes = set()
                for STEP in range(0, 5):
                    next_nei_nodes = get_boundary_nodes(self.CFG, corpus_covered | nei_nodes, node_idx_name_table, node_name_idx_table, self.bb_id_dict)
                    for nei_node_idx in next_nei_nodes:
                        if nei_node_idx not in corpus_covered or (ndatas[nei_node_idx][-2] == 0 and ndatas[nei_node_idx][-3] == 0):
                            ndatas[nei_node_idx][-1] = 1
                    nei_nodes |= next_nei_nodes
                
                dgl_corpus_base = dgl_corpus_base.subgraph(list(corpus_covered - nei_nodes))

                print(dgl_corpus_base)
                
                for seed in tqdm.tqdm(seeds_now):
                    with open(os.path.join(seeds_dir, seed, "last_virgin"), "rb") as lvd:
                        last_virgin = lvd.read()
                    with open(os.path.join(seeds_dir, seed, "first_virgin"), "rb") as fvd:
                        first_virgin = fvd.read()
                        seed_hash = hashlib.sha256(first_virgin).hexdigest()
                    tag = count_bits(combine_map(first_virgin_combine, last_virgin)) / bits_cnt_total

                    dgl_export = dgl_corpus_base.clone()
...
if __name__ == "__main__":
    ...
    cdfg.exportTrainingSet(sys.argv[3], sys.argv[4])
