...
bit_idxs_table = np.array([
    [
        ((i >> k) & 1) for k in range(0, 8)
    ]
...
class CDFG:
    ...
    def preprocessGraph(self):
        ...
        tensor_nodes_features = torch.tensor(nodes_features)
    ...
    def exportGraph(self, virgin_bits, seed_bits, virgin_changed=True):
        ...
        if virgin_changed:
            ...
            #print(max(self.node2oldnode_tbl))

            #print(len(self.dgl_virgin_base.nodes()))
            self.dgl_virgin_base.remove_nodes(torch.tensor(self.masked_nodes))

            for i in range(0, len(self.node2oldnode_tbl)):
                if self.node2oldnode_tbl[i] >= self.dgl_virgin_base.num_nodes():
                    self.node2oldnode_tbl[i] = self.dgl_virgin_base.num_nodes() - 1
            #print(len(self.dgl_virgin_base.nodes()))
            #print(self.node2oldnode_tbl.tolist())
            #input("paused!")
        ...
        try:
            ...
            self.hash_edge_np = np.zeros((len(seed_bits_u8_array) << 3, 2), dtype=np.int32)
            for hash_value, edge in self.hash_edge.items():
                self.hash_edge_np[hash_value][0] = edge[0]
                self.hash_edge_np[hash_value][1] = edge[1]
                
        non_zero_indexes = np.nonzero(seed_bits_u8_array)[0]

        indexes = np.nonzero(bit_idxs_table[seed_bits_u8_array[non_zero_indexes]])
        indexes = np.array(indexes, dtype=np.uint32)
        indexes = (indexes[0] << 3) + indexes[1]

        tmp = ndatas[self.node2oldnode_tbl[self.hash_edge_np[indexes][:,0]]]
        tmp[:,-2] = 0
        ndatas[self.node2oldnode_tbl[self.hash_edge_np[indexes][:,0]]] = tmp

        tmp = ndatas[self.node2oldnode_tbl[self.hash_edge_np[indexes][:,1]]]
        tmp[:,-2] = 0
        ndatas[self.node2oldnode_tbl[self.hash_edge_np[indexes][:,1]]] = tmp
        ...
        dgl_export.ndata['attr'] = torch.tensor(ndatas, dtype=torch.float32)
