...
struct queue_entry {
  u64 virgin_cnt;
  u8 *trace_mini_kept;
  u64 trace_mini_kept_len;
  u8 *attention_nodes;
  u64 selected_log;
  u64 weight_update_time;
  u64 attention_update_time;
  double AI_weight;
  double legacy_weight;
  double AI_weight_norm;
  double attention_score;
  double attention_score_norm;
...
typedef struct afl_state {
  ...
  s32 nn_mem_fd;
  s32 nn_unix_sock;
  double *average_attention;
  u8 *average_attention_scatter;
  u64 average_attention_update_time;

  u64 virgin_cnt;
  u8 virgin_changed;
  u64 virgin_upload_time;

  void *nn_shm;

  u8 *attention_edges;

  char nn_mem_path[1025];
  char nn_mem_name[1025];
  char *nn_unix_name;
