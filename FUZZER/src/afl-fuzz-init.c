...
void perform_dry_run(afl_state_t *afl) {
  ...
  for (idx = 0; idx < afl->queued_items; idx++) {
    ...
    while (1) {
      ...
      if (send(afl->nn_unix_sock, "updt map", strlen("updt map"), 0) < strlen("updt map")) {
        WARNF("updt map send ERROR!");
        reconstruct_conn(afl);
        continue;
      }
      if (read(afl->nn_unix_sock, recv_buf, 2) < 2) {
        WARNF("updt map read ERROR!");
        reconstruct_conn(afl);
        continue;
      }
      minimize_bits(afl, afl->queue_buf[idx]->trace_mini_kept, afl->fsrv.trace_bits);
      ...
      free(afl->queue_buf[idx]->trace_mini_kept);
      afl->queue_buf[idx]->trace_mini_kept = compressed;
      afl->queue_buf[idx]->trace_mini_kept_len = compressed_len;
      ...
      *(size_t *)recv_buf = afl->queue_buf[idx]->trace_mini_kept_len;
      if (send(afl->nn_unix_sock, recv_buf, 8, 0) < 8) {
        WARNF("updt map size send ERROR!");
        reconstruct_conn(afl);
        continue;
      }
      if (read(afl->nn_unix_sock, recv_buf, 2) < 2) {
        WARNF("updt map ok ERROR!");
        reconstruct_conn(afl);
        continue;
      }
      break;
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  if (!afl->in_place_resume) {
    ...
    fprintf(
        ...
        "saved_hangs, max_depth, execs_per_sec, total_execs, edges_found, overhead, exec_total_time\n");
