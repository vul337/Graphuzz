...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
      (opt = getopt(
           argc, argv,
           "+Ab:B:c:CdDe:E:hi:I:f:F:g:G:l:L:m:M:nNOo:p:RQs:S:t:T:UV:WXx:YZK:")) >
      0) {

    switch (opt) {

      case 'K':
        afl->nn_unix_name = optarg;
        break;
  ...
  if (afl->non_instrumented_mode || afl->fsrv.qemu_mode ||
    ...
    afl->attention_edges = ck_realloc(afl->attention_edges, map_size);
    ...
    do {
      ...
      if (likely(!afl->old_seed_selection)) {
        ...
        if (unlikely(prev_queued_items < afl->queued_items ||
          ...
          //WARNF("overhead %lf sec", afl->overhead);
        ...
        //afl->queue_cur->selected_log = log2(pow(2, afl->queue_cur->selected_log) + 1);
        if (afl->queue_cur->AI_weight * 0.995 > 100.0) {
          afl->queue_cur->AI_weight *= 0.995;
        }
