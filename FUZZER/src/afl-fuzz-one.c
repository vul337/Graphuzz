...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (unlikely(afl->not_on_tty)) {
    ...
    ACTF(
        ...
        "exec_us=%llu, hits=%u, map=%u, ascii=%u, legacy_weight=%lf, AI_weight=%lf, AI_weight_norm=%lf, attention_score_norm=%lf)...",
        ...
        afl->queue_cur->legacy_weight, afl->queue_cur->AI_weight,
        afl->queue_cur->AI_weight_norm, afl->queue_cur->attention_score_norm);
    fflush(stdout);
